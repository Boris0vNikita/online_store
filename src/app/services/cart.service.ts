...
export class CartService {
  ...
  removeOnCart(item:CartItem,update = true):Array<CartItem>{
  ...
  removeQuantity(item:CartItem):void{
    ...
    if(itemForRemove){
      filteredItems = this.removeOnCart(itemForRemove,false)
    }
    this.cart.next({items:filteredItems})
    this._snackBar.open('1 item remove from cart','OK',{duration:2000})
  }
}
